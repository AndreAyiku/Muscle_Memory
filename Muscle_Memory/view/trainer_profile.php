...
// Check if a specific trainer ID is provided
$trainer_id = isset($_GET['id']) ? intval($_GET['id']) : (isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : null);

// If no trainer ID, redirect
if (!$trainer_id) {
...
$stmt = $conn->prepare("
    SELECT t.*, u.first_name, u.last_name, u.email, u.profile_picture, u.user_id, u.user_type
    FROM MM_Trainers t
    JOIN MM_Users u ON t.user_id = u.user_id 
    WHERE t.user_id = ?
");
...
$can_edit = (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $trainer_id && $_SESSION['user_type'] === 'trainer');
...
// Prepare specialties and certifications
$secondary_specialties = !empty($profile['secondary_specialties']) 
    ? explode(',', $profile['secondary_specialties']) 
    : [];
$certifications = !empty($profile['certifications']) 
    ? explode(',', $profile['certifications']) 
    : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Muscle Memory - Trainer Profile</title>
    ...
    <style>
        /* Previous CSS remains the same - reusing from earlier implementations */
        body, html {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        <div class="profile-container">
            <div class="profile-header">
                <img src="<?php echo htmlspecialchars($profile_picture); ?>" alt="Trainer Profile" class="profile-picture-large">
                <div class="profile-details">
                    ...
                    <p><i class='bx bx-map'></i> <?php echo htmlspecialchars($profile['consultation_type']); ?> Training</p>
                    
                    <?php 
                    if($can_edit) {
                        echo '<a href="../view/edit_train_profile.php" class="edit-profile-btn">Edit Profile</a>';
                    }
                    ?>
                </div>
            </div>
            ...
            <div class="profile-sections">
                <div class="profile-section">
                    ...
                    <ul>
                        ...
                        <?php 
                        foreach ($secondary_specialties as $specialty) {
    ...
    <div class="profile-sections" style="margin-top: 20px;">
        <div class="profile-section">
            ...
            <p><strong>Rate:</strong> $<?php echo number_format($profile['session_rate'], 2); ?> per session</p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        createBubbles(); // Initial bubble creation
